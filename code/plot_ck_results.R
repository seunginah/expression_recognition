...
f1 <- c('raw pixel', 'pixel_norm1','hog', 'hog_norm1', 'hog_norm2', 'lbp', 'gabor', 'gabor_norm1', 'pixel_sharpen', 'pixel_sharpen_norm1')
f2 <- c('fiducial_points', 'fiducial_points_n1','soft_clustering','soft_clustering_norm2', 'pixel_gradient')

df1 <- results %>% filter(feature %in% f1) %>% arrange(feature)
df1 <- df1[c(19:21, 7:18, 1:6, 22:27), ]
df2 <- results %>% filter(feature %in% f2) %>% arrange(feature)
df2 <- df2[c(1:6, 10:15, 7:9), ]
